...
- current: M1 6/7 已合入（events-client d5362f8，审查零打回）；仅剩 platform-util 在途
- next: platform-util 交卷审查合入 → mig-purge-architectury 门禁（dev.architectury=0 + fabricmc 注解/CapabilityLifecycle 门面/YsmEventBootstrap 空入口清理 + 进世界全量验收）→ M2 1.16.5（unimined）
...
| mig-platform-util | in_progress | work/mig-platform-util | 主类+音效接线+NCDFE P2——M1 最后一支 |
