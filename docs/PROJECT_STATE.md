...
- current: compile-green-gate in_review（新会话①/5，冒烟驱动 4 处语义修复为重点复核项）
- next: 门禁合入 → native-poc（GO/NO-GO）+ ingame-smoke-gate → **M2 收官：记忆整理（蒸馏/kg_stats/锚点）→ 用户压缩上下文 → M3 全谱平铺**
...
| m2-compile-green-gate | in_review | work/m2-compile-green-gate | 7 commits；双版本绿+四内嵌+双冒烟自查过；审查重点=冒烟驱动 4 处语义修复的 1.20.1 零回退 |
