...
- current: M2 推进中——骨架+java8-downshift+gate-compat 三卡均已合入 dev（gate-compat 系打回修复后复验通过）
- next: 条件卡波次（forge-thinlayer/render-pipeline/mixin-versioning 并行）→ gui-hud-port → native-poc(GO/NO-GO) → 双门禁
