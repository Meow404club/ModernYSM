...
- current: 无在途分支；dev 顶=b724e71（RC 三情境旋转收口）；**发布物料就绪**：docs/release/RELEASE-MANIFEST.md（35 线）+CHANGELOG-draft.md，发布取件铁律=versions/<线>/build/libs/<jar>（reobf 物），收集目录 11 件 forge devlibs 已加后缀防误取
- next: **压缩后一次性并行派发候选池全部可做项**（A~H 八项清单+内嵌证据=state:tasks.handoff-2026-09-18 的 batch_dispatch_after_compress）；外部等待项仅 FPM 21.9~21.11+26.3（等上游）
