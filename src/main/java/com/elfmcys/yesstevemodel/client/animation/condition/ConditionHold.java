...
public class ConditionHold {
    ...
    private final ReferenceOpenHashSet<UseAction> extraTes = new ReferenceOpenHashSet<>();
    ...
    public void addTest(String name) {
        ...
        if (!name.startsWith(this.extraPre) || strSubstring.equals(UseAction.NONE.name().toLowerCase(Locale.US))) {
            return;
        }
        Optional<UseAction> optional = EquipmentUtil.getUseAnimByName(strSubstring);
    ...
    private String doTagTest(LivingEntity livingEntity, InteractionHand interactionHand) {
        ...
        return stream.filter(tag -> tag.matches(itemInHand)).findFirst().map(tag -> this.tagPre + tag.location()).orElse("");
    }
    ...
    private String doExtraTest(LivingEntity entity, InteractionHand hand) {
        ...
        UseAction anim = rip.ysm.util.UseAction.of(entity.getItemInHand(hand));
