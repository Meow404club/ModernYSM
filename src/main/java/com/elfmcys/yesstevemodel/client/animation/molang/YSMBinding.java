...
public class YSMBinding extends ContextBinding {
    ...
    private static Object dumpEffects(IContext<Entity> context) {
        ...
        for (MobEffectInstance mobEffectInstance : activeEffects) {
            context.logWarningComponent(YsmText.literal("Effect: display ").append(ComponentUtils.copyOnClickText(mobEffectInstance.getEffect().getDisplayName().getString(99))).append(YsmText.literal("  name ").append(ComponentUtils.copyOnClickText(YsmTag.mobEffectKey(mobEffectInstance.getEffect()).toString()))).append("  lv=").append(String.valueOf(mobEffectInstance.getAmplifier() + 1)));
        }
        return null;
    }
    ...
    private static Object dumpBiome(IContext<Entity> context) {
        ...
        if (biomeName != null) {
            context.logWarningComponent(YsmText.literal("Name ").append(ComponentUtils.copyOnClickText(biomeName.toString())));
        }
         *///?} else {
        Holder<Biome> biome = context.entity().level().getBiome(context.entity().blockPosition());
        biome.unwrapKey().ifPresent(resourceKey -> {
            context.logWarningComponent(YsmText.literal("Name ").append(ComponentUtils.copyOnClickText(resourceKey.location().toString())));
        });
        biome.tags().forEach(tagKey -> {
            context.logWarningComponent(YsmText.literal("Tag ").append(ComponentUtils.copyOnClickText(tagKey.location().toString())));
        });
        //?}
        return null;
    }
