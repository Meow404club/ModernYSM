...
public final class AuthorRow extends OptionRow<Object> {
    ...
    protected void renderWidget(YsmGui g, int mouseX, int mouseY, float partialTick) {
        boolean hover = /*? if >=1.18.2 && <1.19.4 {*/ /*isHoveredOrFocused()*//*?} else {*/ isHovered() /*?}*/;
        ...
        g.drawString(font, YsmGui.text(name).withStyle(ChatFormatting.GOLD, ChatFormatting.BOLD), tx, getY() + 6, -1, false);
        if (StringUtils.isNotBlank(role)) {
            int nameW = font.width(name);
            g.drawString(font, YsmGui.text(role).withStyle(ChatFormatting.GREEN), tx + nameW + 8, getY() + 6, -1, false);
        }
        ...
        if (contacts != null && contacts.size() > 0) {
            ...
            for (int i = 0; i < contacts.size(); i++) {
                ...
                g.drawString(font, YsmGui.text(key).withStyle(ChatFormatting.YELLOW), chipX + 4, chipY + 2, -1, false);
