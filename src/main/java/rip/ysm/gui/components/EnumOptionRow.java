...
public class EnumOptionRow<E extends Enum<E>> extends OptionRow<E> {
    ...
    protected void renderControl(YsmGui g, int mouseX, int mouseY, float partialTick) {
        ...
        Component text = YsmGui.text(prettify(option.get().name()));
    ...
    public void renderOverlay(YsmGui g, int mouseX, int mouseY, float partialTick, float scrollDisplay) {
        ...
        for (int i = 0; i < visible; i++) {
            ...
            g.drawString(Minecraft.getInstance().font, YsmGui.text(prettify(values[idx].name())), listX + 6, itemY + (14 - 8) / 2, -1, true);
        }
