...
/**
 * 刀挂渲染（1.20.1 SlashBladeRenderer 直译）。签名适形点：Capability 体系删除 →
 * BladeStateAccess.of(stack)（上游 BladeStateAccess.java:29）；渲染链
 * BladeModelManager/WavefrontObject/BladeRenderState 全同形态（上游源码逐文件核对，
 * renderOverrided/renderOverridedLuminous/getModel/getInstance 签名未变）。
 * 默认资产 RL（风险点②核对结论）：上游 assets/slashblade/model/blade.obj|blade.png
 * 原样在库（harvest 实证），slashblade:model/blade.obj|png 不变。
 */
...
public final class SlashBladeRenderer {
	...
	public static void renderOnEntity(LivingEntity entity, AnimatedGeoModel model, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, ItemStack stack, float partialTick) {
		if (SlashBladeCompat.isSlashBladeItem(stack)) {
			...
			if (bladeBones.isEmpty() || sheathBones.isEmpty() || leftWaistBones.isEmpty()) {
				renderBladeOnWaist(entity, poseStack, bufferSource, packedLight, stack, partialTick, leftWaistBones);
			} else {
	...
	private static void renderBladeOnWaist(LivingEntity entity, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, ItemStack stack, float partialTick, List<IBone> leftWaistBones) {
		poseStack.pushPose();
		...
		BladeStateAccess.of(stack).ifPresent(bladeState -> {
			...
			long timeSinceLastAction = entity.level().getGameTime() - bladeState.getLastActionTime();
	...
	private static void renderBladeOnly(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, ItemStack stack) {
		if (stack.isEmpty()) {
			return;
		}
		BladeStateAccess.of(stack).ifPresent(bladeState -> {
