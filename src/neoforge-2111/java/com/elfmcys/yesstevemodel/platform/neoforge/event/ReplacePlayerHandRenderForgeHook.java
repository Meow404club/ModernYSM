...
public final class ReplacePlayerHandRenderForgeHook {
    ...
    public static void onRenderArm(RenderArmEvent event) {
        // 1.21.10 RenderArmEvent.getMultiBufferSource 删（getSubmitNodeCollector 替代）→
        // Static 直绘取 renderBuffers 即时缓冲（功能债同 RenderFirstPlayerForgeHook 注）
        if (ReplacePlayerHandRenderEvent.onRenderArm(event.getPlayer(), event.getArm(), event.getPoseStack(), net.minecraft.client.Minecraft.getInstance().renderBuffers().bufferSource(), event.getPackedLight())) {
