...
public final class KeyMappingFactoryImpl {
    ...
    // 1.21.10 KeyMapping 分类记录化：String category → KeyMapping.Category(Identifier)
    //（2110 KeyMapping.java:293 record Category(Identifier id) 实证）
    private static KeyMapping.Category ysmCategory(String category) {
        return new KeyMapping.Category(net.minecraft.resources.Identifier.parse(category));
    }
