...
/**
 * <1.14 轴 molang 命名空间空绑定注入（fix-122-08sta-scale，1710 蓝本 a13b89a 同款
 * twin；GeckoLibCache 为共享类，注入发生在本线运行时进程内，共享源零触碰）。
 *
 ...
 * 08_sta 全骨隐藏 quadsDrawn=0 实证，02-mid-run1.log）。
 *
 * 修法：GeckoLibCache.EXTRA_BINDING 是 private static final 修饰的 HashMap（引用
 * final、内容可变），createMolangParser <1.14 分支每次以 `new HashMap<>(EXTRA_BINDING)`
 * 传给 MolangParser——反射注入一次，所有后续解析获得与主线未赋值态一致的空语义：
...
public final class LegacyMolangNullBindings {
    ...
    private static synchronized void install0() throws ReflectiveOperationException {
        ...
        System.out.println("[ysm-legacy122] molang null bindings installed: namespaces=" + NAMESPACES.length);
    }
    ...
    public static synchronized void install() {
        ...
        try {
            ...
            System.out.println("[ysm-legacy122] molang null bindings install failed: " + e);
        }
    }
}
