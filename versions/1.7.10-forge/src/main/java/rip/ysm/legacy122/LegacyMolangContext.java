...
/**
 * 1.7.10 线 molang 高频变量真值供给（D-molang-1，122 蓝本 1:1 twin）。
 *
 * <p>包名说明：共享 PrimaryBinding 的 <1.14 条件分支硬编码 import
 * rip.ysm.legacy122.LegacyMolangContext——包名是共享解析链既定契约点
 * （stonecutter 只剥条件不换包名，1710 线在本包名下提供同名 shim，Mth 同款先例）。
 *
 * <p>共享侧 PrimaryBinding <1.14 分支正式注册本类（q/query/ysm/ctrl 真值根 +
 * v/variable/t/temp 暂存 + tlm/args/fn/context/c 空链兜底），替代
 * fix-1710-08sta-scale 的 LegacyMolangNullBindings 反射注入（该类已删）。
 * 落地清单/语义对位/适形声明见 122 蓝本类注（builtin 动画 json 频率序）。
 * 1.7.10 面考古（unimined mcp-stable-12 jar javap 亲证）：swing 族
 * EntityLivingBase isSwingInProgress/swingProgressInt/swingProgress/getSwingProgress
 * 公开同面；renderYawOffset/prevRenderYawOffset/rotationYawHead/prevRotationYawHead/
 * yBodyRot(O)/prevRotationPitch 同名；MathHelper 差异面=wrapAngleTo180_float/
 * clamp_float（122 为 wrapDegrees/clamp）。1.7.10 单手域：swinging_arm 恒 0、
 * has_offhand 恒 false、has_mainhand=getHeldItem()（主线 anim2-1710 单手域同款）。
 */
public final class LegacyMolangContext {
    ...
    public static void beginFrame(@Nullable EntityPlayer player, float partialTick) {
        ...
        if (DEBUG && player != null) {
            LOG.info(String.format(
                    "[ysm-legacy1710] molangFrame vanilla-truth: swingProgressInt=%d swingProgress=%.4f hurtTime=%d health=%.1f",
        ...
        QUERY {
            ...
            Map<String, Object> build() {
                ...
                m.put("ground_speed", new Leaf("ground_speed", () -> {
                    ...
                    double dx = p.motionX;
                    double dz = p.motionZ;
                    return 20.0f * MathHelper.sqrt_float((float) (dx * dx + dz * dz));
                }));
                m.put("yaw_speed", new Leaf("yaw_speed", () -> {
                    EntityPlayer p = FRAME.player;
                    return p == null ? null
                            : 20.0f * MathHelper.wrapAngleTo180_float(p.rotationYaw - p.prevRotationYaw);
                }));
                ...
                m.put("is_jumping", new Leaf("is_jumping", () -> {
                    ...
                    EntityPlayer p = FRAME.player;
                    return p != null && !p.capabilities.isFlying && p.riddenByEntity == null
                            && !p.onGround && !p.isInWater();
                }));
                m.put("time_stamp", new Leaf("time_stamp", () -> {
                    EntityPlayer p = FRAME.player;
                    return p == null ? null : (float) p.worldObj.getWorldTime();
                }));
                m.put("body_y_rotation", new Leaf("body_y_rotation", () -> {
                    EntityPlayer p = FRAME.player;
                    // 1.7.10 mcp-stable-12 未命名 yBodyRot(O)（javap 实证）→ 躯干 yaw
                    // 对位 renderYawOffset 插值（RendererLivingEntity 躯干旋转源同字段）
                    return p == null ? null : MathHelper.wrapAngleTo180_float(
                ...
                m.put("has_rider", new Leaf("has_rider", () -> {
                    EntityPlayer p = FRAME.player;
                    return p != null && p.riddenByEntity != null;
                }));
        ...
        YSM {
            ...
            Map<String, Object> build() {
                ...
                m.put("swing_time", new Leaf("swing_time", () -> {
                    ...
                    if (DEBUG) {
                        LOG.info(String.format("[ysm-legacy1710] molang swing_time=%d (swingProgress=%.4f)", v, p.swingProgress));
                    }
                    return (float) v;
                }));
                // 1.7.10 单手域：swingingArm 概念不存在（EnumHand 1.8+），恒主手
                m.put("swinging_arm", new Leaf("swinging_arm", () -> 0.0f));
                m.put("attack_time", new Leaf("attack_time", () -> {
                    ...
                    if (DEBUG) {
                        LOG.info(String.format("[ysm-legacy1710] molang attack_time=%.4f (swingProgressInt=%d)", v, p.swingProgressInt));
                    }
                    return v;
                }));
                ...
                m.put("has_mainhand", new Leaf("has_mainhand", () -> {
                    EntityPlayer p = FRAME.player;
                    // 1.7.10 无 ItemStack.isEmpty，持物=getHeldItem()!=null 且 item 非空
                    return p != null && p.getHeldItem() != null && p.getHeldItem().getItem() != null;
                }));
                // 1.7.10 单手域：副手不存在
                m.put("has_offhand", new Leaf("has_offhand", () -> false));
                m.put("is_close_eyes", new Leaf("is_close_eyes", () -> {
                    // 主线 YSMBinding:439-441 同式（眨眼钟=本线 life tick；适形见 122 类注）
                    EntityPlayer p = FRAME.player;
                    if (p == null) {
                        return null;
                    }
    ...
    /** 本帧主状态机裁决状态名（LegacyWorldAnimationController 每 tick 发布，骑乘=null）。 */
    ...
    // ==================== 插值工具（RendererLivingEntity 同式） ====================

    private static float interp(float prev, float cur, float pt) {
        return prev + (cur - prev) * pt;
    }

    /** 净头偏航：wrap±180(headYaw-yawOffset) ±85 钳（RendererLivingEntity.doRender 同式）。 */
    private static Float headYaw() {
        EntityPlayer p = FRAME.player;
        if (p == null) {
            return null;
        }
        float yawOffset = interp(p.prevRenderYawOffset, p.renderYawOffset, FRAME.partialTick);
        float headYaw = interp(p.prevRotationYawHead, p.rotationYawHead, FRAME.partialTick);
        return MathHelper.clamp_float(MathHelper.wrapAngleTo180_float(headYaw - yawOffset), -85.0f, 85.0f);
    }

    /** 头俯仰=rotationPitch 插值。 */
    private static Float headPitch() {
        EntityPlayer p = FRAME.player;
        return p == null ? null : interp(p.prevRotationPitch, p.rotationPitch, FRAME.partialTick);
    }
